...
export class ApplicationStore {
  ...
  public static template = {
    ...
    add: ({
      ...
      const configs = Object.keys(variants).reduce<Record<string, any>>(
        (res, curr) => {
          res[curr] = {};
          return res;
        },
    ...
    variant: (variant_id: string) => {
      ...
      for (const key in templates) {
        ...
        temp[key] = {
          ...
          configs: {
            ...previous.configs,
            [variant_id]: previous.configs[variant_id] || {},
          },
        };
      }
  ...
  public static config = {
    all: (template_id: string) => {
      const configs = this.template.get(template_id).configs;
    ...
    update: ({
      ...
      const template = this.template.get(template_id);
      const configs = template.configs[variant_id];
      template.configs[variant_id] = {
        ...configs,
        [config_id]: { ...configs[config_id], ...payload },
      };
      this.template.update({ template_id, payload: template });
    },
    delete: ({
  ...
  public static variant = {
    all: () => {
      const variants = this.store.get(StoreKey.Variant);
      return Object.keys(variants).map((key) => variants[key]);
    },
