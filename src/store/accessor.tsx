...
import { _log, _setConfig, _setFilePath, _setState, _setStatus } from "./actioncreators";
...
const ReduxAccessor = () => {
    ...
    const setConfig = useCallback((config: any) => dispatch(_setConfig(config)), [dispatch]);
    ...
    const config: any | null = useSelector((state: ReduxState) => state.config, shallowEqual);
    ...
    return { log, setState, setStatus, setFilePath, setConfig, state, status, filepath, config, logs }
}
