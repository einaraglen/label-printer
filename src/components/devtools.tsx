...
const DevTools = () => {
  ...
  /*const checkInput = (keyword: any, value: any) => {
  ...
  /*const setCommand = (keyword: any, value: any) => {
    ...
    const resetCommand = (keyword: any, value: any) => {
        if (checkInput(keyword, value)) return checkInput(keyword, value);
        try {
  ...
  const exportConfig = async () => {
    //let result = await ipcRenderer.invoke("export-config", JSON.stringify(state.value.config));
    //if (!result || result.path === undefined) return state.method.setOutput((o) => [...o, buildResponse(false, "Could not export, try again")]);
    //state.method.setOutput((o) => [...o, buildResponse(true, "Config was exported!")]);
  };

  const handleInputChange = async (event: any) => {
    if (!event.target.value) return;
    let rawJSON = await readFile(event.target.files[0].path);
    let config = JSON.parse(rawJSON as string);
    //if (!isConfigGood(config, state.value.usableProperties)) return state.method.setOutput((o) => [...o, buildResponse(false, "Bad config file")]);
    ...
    //state.method.setOutput((o) => [...o, buildResponse(true, "Config was imported!")]);
  };

  return (
    <div className="dev-tools">
      <div className="history">
        {/*state.value.output.map((current: any) => current)*/}
        <AlwaysScrollToBottom />
      </div>
      <TextField autoFocus onKeyDown={handleKeyDown} value={commandField} onChange={(event) => setCommandField(event.target.value)} style={{ width: "100%" }} />
      <input ref={inputRef as any} id="file-button" style={{ display: "none" }} accept={".json"} type="file" name="config_file" onChange={handleInputChange} />
    </div>
  );
};
